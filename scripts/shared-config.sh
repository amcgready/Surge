...
propagate_shared_config() {
    ...
    # Update Scanly config
    update_scanly_config
...
# Update Kometa configuration
update_kometa_config() {
    local config_file="$PROJECT_DIR/config/kometa/config.yml"
    local config_dir="$(dirname "$config_file")"
    
    mkdir -p "$config_dir"
    
    cat > "$config_file" << EOF
# Kometa Configuration - Auto-generated by Surge
# Last updated: $(date)
...
# Update Scanly configuration
update_scanly_config() {
    local config_file="$PROJECT_DIR/config/scanly/config.yml"
    local config_dir="$(dirname "$config_file")"
    
    mkdir -p "$config_dir"
    
    cat > "$config_file" << EOF
# Scanly Configuration - Auto-generated by Surge
# Last updated: $(date)

general:
  scan_interval: 3600  # 1 hour
  deep_scan_interval: 86400  # 24 hours
  max_file_age: 2592000  # 30 days
...
database:
  path: /config/scanly.db
  backup_interval: 86400

logging:
  level: INFO
  file: /config/scanly.log
  max_size: 10MB
  backup_count: 5

EOF

    # Add Discord webhook if available and processing notifications enabled
    if [ -n "$DISCORD_WEBHOOK_URL" ] && [ "${DISCORD_NOTIFY_PROCESSING:-false}" = "true" ]; then
        cat >> "$config_file" << EOF
...
notifications:
  discord:
    ...
    title_prefix: "${NOTIFICATION_TITLE_PREFIX:-Surge} - Scanly"

EOF
    fi

    # Add TMDB integration if available
    if [ -n "$TMDB_API_KEY" ]; then
        cat >> "$config_file" << EOF
